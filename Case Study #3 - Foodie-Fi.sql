...
SELECT 
  ...
  ROUND(100.0 * COUNT(customer_id) / (SELECT COUNT(DISTINCT customer_id) FROM foodie_fi.subscriptions WHERE plan_id = 0), 1) AS conversion_percentage
FROM next_plans
...
SELECT 
  ROUND(
    ...
    )::integer, 0) AS avg_days_to_upgrade
...
SELECT 
   CASE 
      WHEN days_diff >= 0 AND days_diff <= 30 THEN '0-30 days'
      WHEN days_diff > 30 AND days_diff <= 60 THEN '31-60 days'
      WHEN days_diff > 60 AND days_diff <= 90 THEN '61-90 days'
...
FROM (
   SELECT 
      s.customer_id,
      MIN(u.start_date) - s.start_date AS days_diff
   FROM 
      subscriptions s
   JOIN 
      subscriptions u ON s.customer_id = u.customer_id
   WHERE 
      s.plan_id = 0
      AND u.plan_id = 3
      AND u.start_date > s.start_date
   GROUP BY 
...
SELECT
    ...
    CASE
        ...
        WHEN p.plan_name LIKE '%monthly' THEN 
            DATE_TRUNC('MONTH', s.start_date) + (EXTRACT(DAY FROM s.start_date) - 1) * INTERVAL '1 DAY'
        -- Pro annual payments
        WHEN p.plan_name = 'pro annual' THEN 
            DATE_TRUNC('YEAR', s.start_date) + INTERVAL '1 YEAR' - INTERVAL '1 DAY'
        -- Churned customers
        ELSE NULL
    END AS payment_date,
    CASE
        -- Upgrades from basic to monthly or pro plans
        WHEN s.start_date = s.start_date AND p.plan_name LIKE '%monthly' THEN 
            p.price - COALESCE((SELECT SUM(price) FROM plans WHERE plan_name LIKE '%monthly' AND start_date < s.start_date AND customer_id = s.customer_id), 0)
        -- Upgrades from pro monthly to pro annual
        WHEN p.plan_name = 'pro annual' THEN 
            p.price - COALESCE((SELECT SUM(price) FROM plans WHERE plan_name LIKE '%monthly' AND start_date < s.start_date AND customer_id = s.customer_id), 0)
        -- Monthly payments and pro annual payments
        ELSE p.price
    END AS amount
FROM
    subscriptions s
...
WHERE
    s.start_date >= '2020-01-01' AND s.start_date < '2021-01-01'
GROUP BY
    s.customer_id,
    s.start_date,
    p.plan_name,
