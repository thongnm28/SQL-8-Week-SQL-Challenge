...
/* 4.How many days on average are customers reallocated to a different node? */
SELECT AVG(EXTRACT(DAY FROM end_date) - EXTRACT(DAY FROM start_date)) AS average_reallocation_days
FROM customer_nodes;

/* 5.What is the median, 80th and 95th percentile for this same reallocation days metric for each region? */
SELECT r.region_name,
       PERCENTILE_CONT(0.5) WITHIN GROUP (ORDER BY EXTRACT(DAY FROM end_date) - EXTRACT(DAY FROM start_date)) AS median_reallocation_days,
       PERCENTILE_CONT(0.8) WITHIN GROUP (ORDER BY EXTRACT(DAY FROM end_date) - EXTRACT(DAY FROM start_date)) AS percentile_80_reallocation_days,
       PERCENTILE_CONT(0.95) WITHIN GROUP (ORDER BY EXTRACT(DAY FROM end_date) - EXTRACT(DAY FROM start_date)) AS percentile_95_reallocation_days
FROM regions r
JOIN customer_nodes cn ON r.region_id = cn.region_id
GROUP BY r.region_name;
...
/* 4.What is the closing balance for each customer at the end of the month? */
SELECT cn.customer_id, DATE_TRUNC('month', cn.end_date) AS month_end,
       COUNT(*) AS closing_balance
FROM customer_nodes cn
GROUP BY cn.customer_id, month_end
ORDER BY cn.customer_id, month_end;

/* 5.What is the percentage of customers who increase their closing balance by more than 5%? */
SELECT COUNT(*) FILTER (WHERE closing_balance_increase > 0.05) * 100.0 / COUNT(*) AS percentage_increase
FROM (
    SELECT cn.customer_id,
           (cn.end_date - cn.start_date) * 100 AS duration_cost,
           (cn.end_date - cn.start_date) * 100 - LAG((cn.end_date - cn.start_date) * 100) OVER (PARTITION BY cn.customer_id ORDER BY cn.start_date) AS closing_balance_increase
